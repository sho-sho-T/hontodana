...
import { transformStatsForChart, formatStatValue, generateChartLabels, calculateTrend } from '@/lib/utils/stats-formatters'
...
describe('transformStatsForChart', () => {
  ...
  describe('P2: チャートオプション設定テスト', () => {
    test('カラーパレットの適用', () => {
      ...
      expect(chartData.datasets[0].backgroundColor).toContain('rgba(59, 130, 246, 0.1)')
    })
...
describe('formatStatValue', () => {
  ...
  describe('P2: 統計値フォーマットテスト', () => {
    ...
    test('パーセンテージの表示フォーマット', () => {
      ...
      expect(formatStatValue(0.001, 'percentage')).toBe('0.1%')
    })
    ...
    test('読書速度の表示フォーマット', () => {
      expect(formatStatValue(1.5, 'speed')).toBe('1.5ページ/分')
      expect(formatStatValue(0.5, 'speed')).toBe('0.5ページ/分')
      expect(formatStatValue(0, 'speed')).toBe('0ページ/分')
      expect(formatStatValue(10.333, 'speed')).toBe('10.3ページ/分')
    })
    ...
    test('無効な値の処理', () => {
      ...
      expect(formatStatValue(NaN, 'percentage')).toBe('0%')
      expect(formatStatValue(-10, 'books')).toBe('0冊') // 負の値は0として処理
    })

    test('カスタムフォーマットオプション', () => {
      expect(formatStatValue(1234.567, 'pages', { precision: 1 })).toBe('1,234.6ページ')
      expect(formatStatValue(0.6789, 'percentage', { precision: 1 })).toBe('67.9%')
  ...
  describe('P2: データフロー統合テスト', () => {
    test('統計データからチャート表示までの完全フロー', () => {
      const rawStats = [
        ...
        { date: '2024-08-19', pagesRead: 35, readingTime: 50, sessionsCount: 2 },
        { date: '2024-08-20', pagesRead: 40, readingTime: 60, sessionsCount: 2 }
      ]

      // データ変換
      const chartData = transformStatsForChart(rawStats, 'pages', { includeTrend: true })
      
      // トレンド計算
      const trend = calculateTrend(chartData.datasets[0].data as number[])
      
      // 統計値フォーマット
      const totalPages = (chartData.datasets[0].data as number[]).reduce((sum, val) => sum + val, 0)
    ...
    test('エラー条件での統合処理', () => {
      const invalidStats = [
        { date: 'invalid', pagesRead: null, readingTime: -10, sessionsCount: undefined }
      ]
