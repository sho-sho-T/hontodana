...
// 構造化ログ関数
function logApiEvent(level: 'info' | 'warn' | 'error', event: string, data: any, requestId: string) {
  const logEntry = {
...
function validateRequestParams(url: URL) {
  ...
  if (timeRange !== null) {
    ...
    if (!validRanges.includes(timeRange)) {
      errors.push(`timeRangeは次のいずれかである必要があります: ${validRanges.join(', ')}`)
    }
  }
  ...
  if (format !== null) {
    ...
    if (!validFormats.includes(format)) {
      errors.push(`formatは次のいずれかである必要があります: ${validFormats.join(', ')}`)
    }
  }
...
export async function GET(request: NextRequest) {
  ...
  // リクエストログ
  logApiEvent('info', 'request_started', {
    method: 'GET',
    url: request.url,
  ...
  try {
    ...
    if (validationErrors.length > 0) {
      logApiEvent('warn', 'validation_error', {
        userId,
    ...
    logApiEvent('info', 'request_completed', {
      userId,
      duration,
      recordCount: Array.isArray(stats.dailyStats) ? stats.dailyStats.length : 0
    }, requestId)
    ...
    if (responseData.meta.estimatedSizeKB > 50) {
      logApiEvent('warn', 'large_response_size', {
        userId,
    ...
    if (error instanceof Error) {
      ...
      if (error.message === 'REQUEST_TIMEOUT') {
        errorResponse = {
          ...
          message: '処理時間が長すぎます。パラメータを簡素化して再試行してください。',
          requestId,
          retryAfter: 30
        }
        statusCode = 408
        ...
        errorResponse = {
          ...
          message: 'データベースに接続できません。しばらく時間をおいて再試行してください。',
          requestId,
          retryAfter: 120
        }
        statusCode = 503
      ...
      logApiEvent('error', 'unknown_error', { error: String(error), duration }, requestId)
    }
