...
export const BookDistributionChart = React.memo<BookDistributionChartProps>(function BookDistributionChart({
  data,
  type,
  title,
  height = 300,
  ...
  const dataHash = useMemo(() => {
    if (!data) return ''
    return JSON.stringify({ data: data.slice(0, 5), length: data.length, type })
  }, [data, type])
  ...
  // チャートオプション
  const options: ChartOptions<'doughnut' | 'bar'> = useMemo(() => {
    const baseOptions = {
      responsive: true,
      ...
      plugins: {
        legend: {
          ...
          labels: {
            ...
            generateLabels: useCallback((chart: any) => {
              const data = chart.data
              if (data.labels.length && data.datasets.length) {
                return data.labels.map((label: string, i: number) => {
                  const value = data.datasets[0].data[i]
                  const percentage = total > 0 ? Math.round((value / total) * 100) : 0
                  const displayLabel = showPercentage ? `${label} (${percentage}%)` : label
        ...
        tooltip: {
          ...
          callbacks: {
            label: useCallback((context: any) => {
              ...
              const actualValue = type === 'doughnut' ? value : value.y || value
      ...
      onClick: useCallback((event: any, elements: any[]) => {
  ...
  }, [title, type, showLegend, showPercentage, total, dataHash, onSegmentClick]) // dataHashを依存関係に変更
  ...
  const updateAccessibility = useCallback(() => {
    ...
    if (chart?.canvas && data) {
      ...
      chart.canvas.setAttribute('aria-label', `書籍分布チャート: ${title || ''} - ${type}形式のデータ可視化`)
      ...
      chart.canvas.setAttribute('aria-describedby', `chart-description-${Math.random().toString(36).substr(2, 9)}`)
    }
            ...
            {data.map((item, index) => {
              ...
              return (
                <div key={`${item.label}-${index}`} className="flex items-center space-x-2">
                  ...
                  <span className="font-medium">{item.value}冊 ({percentage}%)</span>
                </div>
              )
            })}
          </div>
