...
export const ReadingProgressChart = React.memo<ReadingProgressChartProps>(
	function ReadingProgressChart({
		...
		const tooltipLabelCallback = useCallback(
			(context: any) => {
				...
				const unit =
					type === "pages" ? "ページ" : type === "sessions" ? "セッション" : "";
		...
		const options: ChartOptions<"line"> = useMemo(
			...
			[title, type, tooltipLabelCallback]
		); // tooltipLakを依存関係に追加
		...
		const updateAccessibility = useCallback(() => {
			...
			if (chart?.canvas && data) {
				...
				chart.canvas.setAttribute(
					"aria-label",
					`読書進捗チャート: ${title || ""} - ${type}データの時系列グラフ`
				);
				...
				if (latestValue) {
					...
					const value =
						type === "pages"
							? latestValue.pagesRead
							: type === "minutes"
								? latestValue.readingTime
					...
					summaryElement.textContent = `最新のデータ: ${value}${unit}. 全${data.length}日間のデータを表示しています。`;
				}
			}
		}, [title, type, data]);
