...
export function SearchResults({ 
  ...
  if (isLoading) {
    return (
      <div data-testid="search-loading" className="flex justify-center items-center py-8">
    ...
    return (
      <div data-testid="no-results" className="text-center py-12">
        <div className="text-gray-500 text-lg">
          {query.trim() ? (
            <>
              「<span className="font-medium">{query}</span>」の検索結果が見つかりませんでした。
    ...
    <div data-testid="search-results" className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-center justify-between text-sm text-blue-800">
          <div>
            {total}件中 {(page - 1) * Math.max(1, books.length)} - {Math.min(page * books.length, total)}件を表示
          </div>
          {query.trim() && (
            <div>
...
function SearchBookCard({ book, query, onStatusChange, onRemove }: SearchBookCardProps) {
    ...
    >
      ...
      <div className="p-4 space-y-3">
        ...
        <p className="text-sm text-gray-600 line-clamp-1">
          <HighlightedText text={book.book.authors.join(', ')} highlight={query} />
        </p>
        ...
        <div className="flex items-center justify-between">
          <span className="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
            {book.status === 'reading' ? '読書中' : 
             ...
             book.status === 'want_to_read' ? '読みたい' : book.status}
          </span>
        </div>
        ...
        {book.status === 'reading' && book.book.pageCount && (
          <div className="w-full bg-gray-200 rounded-full h-2">
            ...
            <div className="text-xs text-gray-500 mt-1">
              {book.currentPage}/{book.book.pageCount} ページ ({Math.round(progressPercentage)}%)
            </div>
          </div>
        )}
