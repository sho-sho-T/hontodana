...
import type { ReadingStats, ReadingStatsOptions, DailyStats, WeeklyStats, ReadingSpeed, MonthlyStats } from '@/lib/models/reading-progress'
 ...
 * キャッシュキーの生成
 */
function generateCacheKey(prefix: string, userId: string, options?: any): string {
 ...
 */
function safeCalculate(numerator: number, denominator: number, precision = 1): number {
...
export async function generateReadingStats(
  ...
  try {
    ...
    const averageSessionTime = safeCalculate(totalReadingTime, validSessions.length, 0)
    const averagePagesPerSession = safeCalculate(totalPagesRead, validSessions.length, 1)
    ...
    const averageBookLength = safeCalculate(totalCompletedPages, completedBooks.length, 0)
 ...
 * キャッシュ統計情報を取得（デバッグ用）
 */
export function getCacheStats(): { size: number; keys: string[]; memory: string } {
  ...
  const totalSize = Array.from(statsCache.values())
    .reduce((acc, cached) => acc + roughSizeOfObject(cached.data), 0)
...
export async function calculateReadingSpeed(userId: string): Promise<ReadingSpeed> {
  ...
  try {
    ...
    for (const session of sessionsData) {
      ...
      // 異常値チェックとフィルタリング
      if (speed > 0 && speed < 50 && isFinite(speed)) { // 50ページ/分以上は異常値
        speeds.push(speed)
      }
    }
    ...
    const outliers = speeds.filter(speed => speed < lowerBound || speed > upperBound)
    const validSpeeds = speeds.filter(speed => speed >= lowerBound && speed <= upperBound)
    ...
    const result = {
      averageSpeed: safeCalculate(validSpeeds.reduce((sum, speed) => sum + speed, 0), validSpeeds.length, 2),
    ...
    // エラー時も短時間キャッシュ
    setCachedData(generateCacheKey('reading-speed', userId), defaultResult, 60 * 1000)
    return defaultResult
  }
}
...
export async function generateMonthlyStats(userId: string, months: number): Promise<MonthlyStats[]> {
  ...
  try {
    ...
    for (const range of dateRanges) {
      // 該当月のセッションをフィルタリング
      const monthSessions = allMonthSessions.filter(session => 
      ...
      const { monthPagesRead, monthReadingTime } = monthSessions.reduce(
        (acc, session) => ({
          ...
          monthReadingTime: acc.monthReadingTime + (session.durationMinutes || 0)
        }),
    ...
    const defaultStats = Array(months).fill(null).map((_, i) => {
      const monthStart = new Date()
      monthStart.setMonth(monthStart.getMonth() - i)
      return {
