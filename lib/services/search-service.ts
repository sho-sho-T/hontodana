 ...
 * 書籍を検索する
 */
export async function searchBooks(params: SearchBooksParams): Promise<SearchResult<UserBookWithBook>> {
  ...
  try {
    ...
    if (hasProgressFilter) {
      books = books.filter(userBook => {
        ...
        const progressPercentage = (userBook.currentPage / userBook.book.pageCount) * 100;
        
        if (filters.progressMin !== undefined && progressPercentage < filters.progressMin) {
          return false;
        }
        
        if (filters.progressMax !== undefined && progressPercentage > filters.progressMax) {
          return false;
        }
 ...
 */
export async function searchBooksAdvanced(params: SearchBooksParams): Promise<SearchResult<UserBookWithBook>> {
  const { query = '', filters = {}, userId, page = 1, limit = 20 } = params;
